...
export class GithubAgent {
	...
	async find_repo(org: string, prompt: string) {
		...
		const { text } = await generateText({
			model: this.model,
			system: `A developer describes a task which is about a repository in his github organization. Select the repository he is most likely talking about and give me only its name. This list of posible repos is the following: ${JSON.stringify(
				repositories
			)}`,
			prompt,
		});
	...
	async summarizeReleases(prompt: string, org: string) {
		...
		let releases = (
			await this.github.queryLatestReleases(org, repo, new Date(since))
		)
		...
		return await Promise.all(
			releases.slice(0, -1).map(async (release, i) => {
				...
				let summary = await this.summarizeRelease(
					org,
					repo,
					release.tag,
					previousRelease
