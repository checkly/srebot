...
export class GithubAgent {
  ...
  async find_repo(org: string, prompt: string) {
    let repositories = (await this.github.queryRepositories(org)).map(r => ({name: r.name, description: r.description, link: r.html_url}));
  ...
  async summarizeReleases(prompt: string, org: string) {
    ...
    let releases = (await this.github.queryLatestReleases(org, repo.name, new Date(since))).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
    .slice(0, 5);

    let summaries = await Promise.all(releases.slice(0, -1).map(async (release, i) => {
      let previousRelease = releases[i + 1]?.tag || '';
      let { diff, summary} = await this.summarizeRelease(org, repo.name, release.tag, previousRelease);
