...
function getFormattedMessages(
  ...
  return messages
    ...
    .map(
      (m) =>
        `<message>${convertSlackTimestamp(
          m.ts!
        ).toISOString()} ${generateSlackMessageLink(m)} Text: ${
          m.plaintext
        }</message>`
    )
    .join("\n");
...
export const analyseAlert = async (
  ...
  const affectedComponentsOutput = await generateObject({
    model: model,
    system:
      "You are an experienced on-call engineer who is responsible for determining which system components are affected by an alert",
    ...
    experimental_telemetry: {
      isEnabled: true,
      functionId: "analyseAlert.affectedComponentsOutput",
    },
  });
  ...
  const recommendationOutput = await generateObject({
    ...
    ${confidentAffectedComponents
      .map(
        (affected) =>
          `Component: ${affected.component} in environment: ${affected.environment}`
      )
      .join("\n")}
    ...
    experimental_telemetry: {
      isEnabled: true,
      functionId: "analyseAlert.recommendationOutput",
    },
  });
  ...
  const affectedComponentsWithRepoNonDev = confidentAffectedComponents.filter(
    (affected) => affected.environment !== "development" && !affected.repository
  );
  ...
  const historyOutput = await generateObject({
    ...
    experimental_telemetry: {
      isEnabled: true,
      functionId: "analyseAlert.historyOutput",
    },
  });
  ...
  const severityOutput = await generateObject({
    ...
    experimental_telemetry: {
      isEnabled: true,
      functionId: "analyseAlert.severityOutput",
    },
  });
  ...
  const summary = await generateObject({
    ...
    experimental_telemetry: {
      isEnabled: true,
      functionId: "analyseAlert.summary",
    },
  });
